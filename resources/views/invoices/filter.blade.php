                    ...
                    <div class="form-row">
                        <div class="form-group col-md-2">
                            ...
                            <input type="date" class="form-control " name="date_from"
                                   value="{{ $request['date_from'] ? $request['date_from'] : now()->addYear(-1)->format('Y-m-d') }}"
                            >
                        </div>
                        <div class="form-group col-md-2">
                            <label for="date_to">@lang('created_to')</label>
                            <input type="date" class="form-control " name="date_to"
                                   value="{{ $request['date_to'] ? $request['date_to'] : now()->format('Y-m-d') }}"
                            >
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="cb_paid" value="paid" name="status[]"
                                @if(in_array('paid', $request['status'])) checked @endif
                            >
                            <label class="form-check-label" for="cb_paid">@lang('paid')</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="cb_not_paid" value="not paid" name="status[]"
                                   @if(in_array('not paid', $request['status'])) checked @endif
                            >
                            <label class="form-check-label" for="cb_not_paid">@lang('not_paid')</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="cb_in_progress" value="in progress" name="status[]"
                                   @if(in_array('in progress', $request['status'])) checked @endif
                            >
